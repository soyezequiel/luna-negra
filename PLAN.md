...
> escrow, multijugador, NIP-46) **ya están implementadas**. Para el
...
- ✅ Custodia de claves Nostr / NIP-46 (firmador en el celu) — **implementado**.
- ⚠️ Email + Magic Link (cuentas custodiales): hay código (`/api/auth/email`) pero **no está operativo** — el login por email no funciona hoy.
...
> pública). El login además de NIP-07 admite **NIP-46** (firmador en el celu por QR).

```
Navegador (Jugador)                         Proveedor
  │  Next.js (App Router)                      │  hostea su juego en su dominio
  │  - login NIP-07 / NIP-46                    │  (opcional) consume API de entitlements
...
- **Auth:** `jose` (JWT). Login estilo **NIP-98**: reto/nonce → firma (NIP-07 o NIP-46) → verificación → cookie de sesión httpOnly. (Hay un flujo de email/magic link en código pero **no operativo**.)
...
2. Perfil: se lee kind:0 del usuario de relays públicos (nombre, avatar, bio) y se cachea.

> Existe un flujo de **email/magic link** (`/api/auth/email`, cuentas custodiales) en
> código, pero **no está operativo**: hoy no se puede iniciar sesión con email.
